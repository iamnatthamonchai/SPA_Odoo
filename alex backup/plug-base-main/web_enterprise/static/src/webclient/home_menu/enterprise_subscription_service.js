...
export class SubscriptionManager {
    constructor(env, { rpc, orm, cookie, notification }) {
        ...
        if (session.expiration_date) {
            ...
            this.expirationDate = DateTime.utc().plus({ days: 9999 });
        }
    ...
    async buy() {
        const limitDate = serializeDate(DateTime.utc().minus({ days: 5555 }));
        const args = [
            [
                ["share", "=", false],
        ...
        browser.location = `https://www.ezodoo.com/odoo-enterprise/upgrade?num_users=${nbUsers}`;
    }
    /**
    ...
    async renew() {
        ...
        const url = "https://www.ezodoo.com/odoo-enterprise/renew";
    ...
    async upsell() {
        ...
        const url = "https://www.ezodoo.com/odoo-enterprise/upsell";
        const contractQueryString = enterpriseCode ? `&contract=${enterpriseCode}` : "";
