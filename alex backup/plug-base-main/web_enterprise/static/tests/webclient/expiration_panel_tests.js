...
QUnit.module("web_enterprise", function ({ beforeEach }) {
    ...
    QUnit.test("Expiration Panel one app installed", async function (assert) {
        ...
        patchDate(9999, 9, 10, 12, 0, 0);

        await createExpirationPanel({
            session: {
                expiration_date: "9999-11-09 12:00:00",
                expiration_reason: "",
    ...
    QUnit.test("Expiration Panel one app installed, buy subscription", async function (assert) {
        ...
        await createExpirationPanel({
            session: {
                expiration_date: "9999-10-24 12:00:00",
        ...
        assert.strictEqual(
            browser.location,
            "https://www.ezodoo.com/odoo-enterprise/upgrade?num_users=7"
        );
    });
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await createExpirationPanel({
                session: {
                    expiration_date: "9999-10-15 12:00:00",
                ...
                mockRPC(route, { args }) {
                    if (route === "/web/dataset/call_kw/ir.config_parameter/get_param") {
                        ...
                        if (callToGetParamCount <= 3) {
                            return "9999-10-15 12:00:00";
                        } else {
                            return "9999-11-15 12:00:00";
                        }
                    }
    ...
    QUnit.test("One app installed, database expired", async function (assert) {
        ...
        await createExpirationPanel({
            session: {
                expiration_date: "9999-10-08 12:00:00",
                expiration_reason: "trial",
                notification_type: true, // used by subscription service to know whether mail is installed
                warning: "admin",
            },
            mockRPC(route, { args, method }) {
                ...
                if (method === "get_param") {
                    ...
                    callToGetParamCount++;
                    if (callToGetParamCount === 1) {
                        return "9999-10-09 12:00:00";
                    } else {
                        return "9999-11-09 12:00:00";
                    }
                }
                return true;
            },
    ...
    QUnit.test("One app installed, renew", async function (assert) {
        ...
        await createExpirationPanel({
            session: {
                expiration_date: "9999-10-20 12:00:00",
    ...
    QUnit.test("One app installed, upgrade database", async function (assert) {
        ...
        assert.strictEqual(
            browser.location,
            "https://www.ezodoo.com/odoo-enterprise/upsell?num_users=13&contract=ABC"
        );
    });
    ...
    QUnit.test("One app installed, message for non admin user", async function (assert) {
        ...
        await createExpirationPanel({
            session: {
                expiration_date: "9999-11-08 12:00:00",
                expiration_reason: "",
                notification_type: true, // used by subscription service to know whether mail is installed
                warning: "user",
            },
    ...
    QUnit.test("One app installed, navigation to renewal page", async function (assert) {
        ...
        assert.strictEqual(
            browser.location,
            "https://www.ezodoo.com/odoo-enterprise/renew?contract=ABC"
        );
