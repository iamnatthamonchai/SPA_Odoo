    ...
    def _compute_late_early_hours(self):
        for rec in self:
            ...
            if calendar and rec.check_in and rec.check_out:
                ...
                local_date = timezone('UTC').localize(rec.check_in).astimezone(timezone(tz))
                ...
                for day in days_to_hours:
                    ...
                    out_min = int((out_hour-int(out_hour)) * 60)
                    check_out = user_timezone.localize(rec.check_out.replace(hour=out_hour1, minute=out_min)).astimezone(timezone('Africa/Tunis'))
