    ...
    def try_cash_in_out(self, session, _type, amount, reason):
        if _type == 'in':
            self.env['sh.cash.in.out']\
                .create({'sh_amount': amount, 'sh_reason': reason, 'sh_session': session, 'sh_transaction_type': 'cash_in'})
        else:
            self.env['sh.cash.in.out']\
                .create({'sh_amount': amount, 'sh_reason': reason, 'sh_session': session, 'sh_transaction_type': 'cash_out'})
