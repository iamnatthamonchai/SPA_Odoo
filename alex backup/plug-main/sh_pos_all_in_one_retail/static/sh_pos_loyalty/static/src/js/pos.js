odoo.define("sh_pos_loyalty.pos", function (require) {
                                ...
                                if (self.env.pos.db.loyalty_by_id[self.env.pos.config.sh_loyalty_rule[0]].sh_loyalty_reward.length == 1) {
                                    if (reward.sh_reward_type == 'gift') {
                                        ...
                                        if (confirmed) {
                                            ...
                                            if (product) {
                                                if(self.env.pos.get_order().get_applied_reward() && self.env.pos.get_order().get_applied_reward().length > 0 && self.env.pos.get_order().get_applied_reward().includes(reward.id)){
                                                    alert("This reward is already applied on this order.")
                                                }else{
                                                    self.env.pos.get_order().set_applied_reward(reward.id)
            ...
            if (radioValue) {
                if (radioValue == 'gift') {
                    ...
                    if (product) {
                        ...
                        _.each(reward1, function (each_reward) {
                            if (each_reward.sh_product_ids[0] == product.id) {
                                if(self.env.pos.get_order().get_applied_reward() && self.env.pos.get_order().get_applied_reward().length > 0 && self.env.pos.get_order().get_applied_reward().includes(each_reward.id)){
                                    alert("This reward is already applied on this order.")
                                }else{
                                    self.env.pos.get_order().set_applied_reward(each_reward.id)
                    ...
                    if (self.env.pos.db.discount_reward) {
                        ...
                        var orderlines = self.env.pos.get_order().get_orderlines()
                        var point_cost = self.env.pos.get_order().get_reward_point()
                        if(self.env.pos.get_order().get_applied_reward() && self.env.pos.get_order().get_applied_reward().length > 0 && self.env.pos.get_order().get_applied_reward().includes(discount_reward[0].id)){
                            alert("This reward is already applied on this order.")
                        }else{
                            self.env.pos.get_order().set_applied_reward(discount_reward[0].id)
                            self.env.pos.get_order().set_reward_point(point_cost + discount_reward[0].sh_point_cost)
                            _.each(orderlines, function (each_order_line) {
                                each_order_line.set_discount(parseFloat(discount_reward[0].sh_discount_percen));
                            });
    ...
    models.Order = models.Order.extend({
        ...
        set_applied_reward: function (apply_reward) {
            if(apply_reward){
                this.apply_reward.push(apply_reward)
            }else{
                this.apply_reward = []
            }
        },
        get_applied_reward: function () {
        ...
        set_client: function (client) {
            if(this.pos.get_order()){    		
        		if(this.pos.get_order().get_client() != client){
                    this.pos.get_order().set_applied_reward(false)
            ...
            if (self.pos.get_order()) {
                if (self.pos.config.sh_enable_coupon && self.pos.db.cart_coupon && self.pos.db.cart_coupon.length > 0 && self.pos.get_order().get_client()) {
                    ...
                    if (orderlines) {
                        _.each(orderlines, function (each_order_line) {
                            if(each_order_line.get_discount()){
                                each_order_line.set_discount(each_order_line.get_discount())
                            }else{
                                each_order_line.set_discount(0)
                            }
                        });
