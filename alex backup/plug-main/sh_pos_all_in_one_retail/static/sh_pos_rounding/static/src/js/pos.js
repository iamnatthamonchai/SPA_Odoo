    ...
    const RoundingPaymentScreen = (PaymentScreen) =>
        class extends PaymentScreen {
            ...
            async validateOrder(isForceValidate) {
                ...
                var is_rounding_added = false
                await _.each(order.get_orderlines(), function (each_line) {
                    if (each_line.product.is_rounding_product){
                        is_rounding_added = true
                        var rounding_price = order.get_round_total_with_tax() - order.get_total_with_tax();
                        order.set_rounding_price(rounding_price);
                        each_line.set_unit_price(rounding_price)
                    }
                })
                
                if (order.get_is_payment_round() && !is_rounding_added) {
                    var rounding_price = order.get_round_total_with_tax() - order.get_total_with_tax();
                    order.set_rounding_price(rounding_price);
...
odoo.define("sh_pos_rounding.OrderWidget", function (require) {
    ...
    const RoundingOrderWidget = (OrderWidget) =>
        class extends OrderWidget {
            ...
            _updateSummary() {
                if (this.env.pos.config.sh_enable_rounding && this.order) {
