...
class ResConfigSettings(models.TransientModel):
    ...
    def _check_boomcast(self):
        result = self.env['ir.module.module'].search(
            [('name', '=', 'boomcast_gateway')])
        if result:
            return True
        else:
            return False        
    ...
    module_boomcast_gateway = fields.Boolean(
        string='Install Boomcast SMS Gateway', help='It will Install Boomcast sms gateway automatically.', default=False)
    is_boomcast_in_addon = fields.Boolean(default=_check_boomcast)
